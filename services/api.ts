...
export const api = {
  ...
  getInteractionsByProspectId: (prospectId: string): Promise<Interaction[]> => mockApiCall(resolve => {
      const prospectInteractions = interactions.filter(i => i.relatedTo === prospectId);
      resolve(prospectInteractions);
  }),
  ...
  addMember: (memberData: Omit<Member, 'id' | 'createdAt' | 'updatedAt' | 'branch'>): Promise<Member> => mockApiCall(resolve => {
  ...
  addTask: (taskData: Omit<Task, 'id' | 'status'>): Promise<Task> => mockApiCall(resolve => {
    const newTask: Task = {
      ...taskData,
      id: `task_${Date.now()}`,
