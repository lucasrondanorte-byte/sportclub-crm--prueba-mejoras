...
import { Task, User, Role, Prospect, Member, TaskType } from '../../types';
...
const CreateTaskModal: React.FC<CreateTaskModalProps> = ({ isOpen, onClose, onSuccess, sellers, prospects, members }) => {
  ...
  const filteredProspectsForModal = useMemo(() => {
    if (assignmentMode === 'massive' && !formData.assignedTo) {
        return [];
    }
  ...
  }, [prospects, prospectStageFilter, prospectSourceFilter, formData.assignedTo, assignmentMode]);
  ...
  const validateForm = (): boolean => {
    ...
    if (assignmentMode === 'single' && !formData.relatedTo) {
      ...
      newErrors.massAssignment = 'Debe seleccionar al menos un prospecto.';
    }
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
        setSelectedRelatedIds(filteredProspectsForModal.map(p => p.id));
    } else {
        setSelectedRelatedIds([]);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (assignmentMode === 'single') {
            ...
            const taskPromises = selectedRelatedIds.map(prospectId => 
                api.addTask({ ...taskData, relatedTo: prospectId })
            );
                           ...
                           <div className="border-t border-brand-border pt-4 flex-1 flex flex-col">
                              <div className="flex justify-between items-center mb-2">
                                  <label className="flex items-center text-sm font-medium text-brand-text-secondary">
                                      <input type="checkbox" className="h-4 w-4 rounded border-brand-border bg-brand-surface text-brand-primary focus:ring-brand-primary" checked={filteredProspectsForModal.length > 0 && selectedRelatedIds.length === filteredProspectsForModal.length} onChange={handleSelectAll} disabled={filteredProspectsForModal.length === 0} />
                                      <span className="ml-2">Seleccionar todos ({filteredProspectsForModal.length})</span>
                                  </label>
                               ...
                               <div className="flex-1 max-h-48 overflow-y-auto border border-brand-border rounded-md bg-brand-bg p-2 space-y-1">
                                  {filteredProspectsForModal.length > 0 ? (
                                      filteredProspectsForModal.map(p => (
                                      ...
                                      <p className="text-center text-sm text-brand-text-muted py-4">No hay prospectos que coincidan con los filtros para el vendedor seleccionado.</p>
                                  )}
                               </div>
