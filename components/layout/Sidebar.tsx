...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setCurrentView, sidebarOpen, setSidebarOpen }) => {
  ...
  return (
    <>
        ...
        <div className={`fixed inset-y-0 left-0 z-30 w-64 flex flex-col bg-brand-sidebar text-brand-text-secondary shadow-lg border-r border-brand-border transition-transform transform lg:translate-x-0 lg:static lg:inset-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="flex items-center justify-center p-6 border-b border-brand-border">
                <LogoIcon className="w-12 h-12" />
                <span className="ml-3 text-2xl font-bold text-brand-text-primary">SportClub CRM</span>
            </div>
            <nav className="p-4 flex-1">
                {navItems.map(item => (
                    user && item.roles.includes(user.role) && (
                        <div key={item.name} className="relative mt-2 group">
                            <a
                                href="#"
                                className={`flex items-center w-full px-4 py-3 rounded-lg transition-colors duration-200 ${currentView === item.name ? 'bg-brand-primary text-white shadow-md' : 'hover:bg-gray-700 hover:text-brand-text-primary'}`}
                                onClick={(e) => {
                                    e.preventDefault();
                            ...
                            >
                             ...
                             <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 z-40 px-3 py-1.5 bg-gray-800 text-white text-xs rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
                                {item.label}
            ...
            <div className="p-4 border-t border-brand-border">
                <div className="relative group">
                    <a
                        ...
                        className="flex items-center w-full px-4 py-3 text-brand-text-secondary rounded-lg transition-colors duration-200 hover:bg-gray-700 hover:text-brand-text-primary"
                    >
                        <GithubIcon />
